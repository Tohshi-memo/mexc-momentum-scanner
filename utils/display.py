...
def print_analysis_result(symbol: str, rsi: float | None, bb_upper: float | None,
                           price: float, is_confirmed: bool) -> None:
    """テクニカル分析結果を1行で出力する。"""
    rsi_str = f"{rsi:.1f}" if rsi is not None else " N/A"
    bb_str  = f"${bb_upper:.6g}" if bb_upper is not None else "N/A"
    ...
    console.print(
        f"  {mark}  [bright_cyan]{symbol:<28}[/bright_cyan]"
        f"  [dim]RSI[/dim] [bright_yellow]{rsi_str}[/bright_yellow]"
        f"  [dim]BB[/dim] {'[bright_red]BREAK ↑[/bright_red]' if bb_upper and price > bb_upper else '[dim]  ok  [/dim]'}"
        f"  [dim]@${price:.6g}[/dim]"
    )
